...
function set_window_title!(win::Window, title::AbstractString, alignment::Symbol)
    ...
    if win.has_border
        ...
        if tw_esc_title > 0
            ...
            @nstyle get_style(theme, :title) win.view begin
...
function _draw_scrollbar!(window::Window)
    ...
    draw_hsb && @nstyle get_style(window.theme, :border) view begin
        for x in 1:hsb_size
            NCurses.mvwprintw(
                view,
                nlines + 1,
    ...
    draw_vsb && @nstyle get_style(window.theme, :border) view begin
        for y in 1:vsb_size
            NCurses.mvwprintw(
                view,
                vsb_pos + y,
