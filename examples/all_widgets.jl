...
function windows_and_widgets()
    ...
    w,c  = create_window_with_container(border = true,
                                        title = "All widgets")

    label = @tui_label(parent = c,
                       left = 0,
                       top = 0,
                       text = "This is a label.")

    ansi_label = @tui_ansi_label(parent = c,
                                 anchor_left = (label, :left),
                                 anchor_top = (label, :bottom),
                                 text = "\e[33m\e[1mThis is an ANSI label.")

    button_c = @tui_container(parent = c,
                              border = true,
                              anchor_left = (label, :left),
    ...
    button_1 = @tui_button(parent = button_c,
                           left = 1,
                           top = 1,
                           label = "Button 1")

    button_2 = @tui_button(parent = button_c,
                           anchor_left = (button_1, :right, 1),
                           anchor_top = (button_1, :top),
                           label = "Button 2")

    button_3 = @tui_button(parent = button_c,
                           anchor_left = (button_2, :right, 1),
                           anchor_top = (button_2, :top),
                           label = "Button 3")

    canvas = @tui_canvas(parent = c,
    ...
    combo_box = @tui_combo_box(parent = c,
                               anchor_right = (:parent, :right),
                               anchor_top = (:parent, :top),
                               data = ["Data $i" for i = 1:20])

    linput_field = @tui_label(parent = c,
                              anchor_left = (label, :left),
                              anchor_top = (button_c, :bottom, 1),
                              text = "Input field")

    input_field = @tui_input_field(parent = c,
                                   anchor_left = (linput_field, :right),
                                   anchor_right = (:parent, :right),
                                   anchor_top = (button_c, :bottom),
                                   border = true)

    list_box = @tui_list_box(parent = c,
                             anchor_left = (label, :left),
    ...
    radio_button_c = @tui_container(parent = c,
                                    border = true,
                                    anchor_left = (label, :left),
                                    anchor_right = (:parent, :right),
    ...
    progress_bar = @tui_progress_bar(parent = c,
                                     anchor_left = (label, :left),
    ...
    for i = 1:2
        anchor_left = i == 1 ? Anchor(:parent, :left, 0) :
                               Anchor(radio_i, :right, 0)

        radio_i = @tui_radio_button(parent = radio_button_c,
        ...
        for j = 2:3
            radio_j = @tui_radio_button(parent = radio_button_c,
